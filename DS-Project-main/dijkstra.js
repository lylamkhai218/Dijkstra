...
const addEdges = () => {
  if (cnt < 2) {
    alert("Bạn phải tạo ít nhất 2 đỉnh");
    return;
  }
...
const indicatePath = async (parentArr, src, minCost) => {
  ...
  for (i = 0; i < cnt; i++) {
    let p = document.createElement("p");
    p.innerText = "Đỉnh " + i + " --> Đường đi ngắn nhất: " + src;
    await printPath(parentArr, i, p);
    p.innerText = p.innerText + "---> Giá đường đi ngắn nhất: " + minCost[i];
  }
}; //Result
